...
def matrix_mul(m_a, m_b):
    ...
    if not m_a or not m_b or not all(row for row in m_a) or \
            not all(row for row in m_b):
        raise ValueError("m_a can't be empty or m_b can't be empty")

    if not all(isinstance(el, (int, float)) for row in m_a for el in row) or \
            not all(isinstance(el, (int, float)) for row in m_b for el in row):
